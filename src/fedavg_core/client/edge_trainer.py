...
def compute_batch_loss(model: torch.nn.Module, batch_features: torch.Tensor, batch_labels: torch.Tensor) -> torch.Tensor:
    ...
    """
    # Forward Pass: Run the features through the model to get raw, unnormalized logits.
    # Shape of logits: (B, num_classes)
    logits = model(batch_features)
    
    # Compute Loss: F.cross_entropy expects raw logits and integer class labels.
...
def train_client_local(
    ...
    """
    # Instantiate the optimizer once, binding it to the model's parameters
    optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate)
    
    # Ensure the model is in training mode (enables dropout/batchnorm if added later)
    model.train()
    
    # Iterate through the requested number of local epochs
    for epoch in range(local_epochs):
        
        # Reshuffle and batch the data predictably but uniquely per epoch
        epoch_seed = seed + epoch
        batches = iterate_client_batches(client_features, client_labels, batch_size, epoch_seed)
        
        # Process every batch using our atomic SGD step
        for batch_features, batch_labels in batches:
            local_sgd_step(model, optimizer, batch_features, batch_labels)
            
    # Extract and return the raw weights payload for network transmission
