...
def local_sgd_step(model: torch.nn.Module, optimizer: torch.optim.Optimizer, batch_features: torch.Tensor, batch_labels: torch.Tensor) -> float:
    ...
    """
    # Clear the gradient buffers from the previous step
    optimizer.zero_grad()
    ...
    # Forward pass: compute the loss and build the computation graph
    # We reuse the compute_batch_loss function built in Step 8
    loss = compute_batch_loss(model, batch_features, batch_labels)
    
    # Backward pass: traverse the graph and populate .grad attributes
    loss.backward()
    
    # Optimizer step: mutate the model parameters in-place down the gradient
    optimizer.step()
    
    # Extract the raw float to prevent memory leaks and allow garbage collection of the DAG
    return loss.item()